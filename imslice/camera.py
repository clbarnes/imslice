...
class Camera:
    ...
    @property
    def normal(self):
        return np.cross(self.top_right - self.top_left, self.bottom_left - self.top_left)
    ...
    def coords(self):
        ...
        top_row = np.linspace(
            self.top_left, self.top_right, self.width, False, axis=1
        )
    ...
    def set_scale_level(self, scale_level: float):
        half_width_vec = self.width * scale_level * unitize(self.top_right - self.top_left) / 2
        half_height_vec = self.height * scale_level * unitize(self.bottom_left - self.top_left) / 2
    ...
    def shape_viewport(self, width=None, height=None):
        ...
        half_width_vec = width * scale_level * unitize(self.top_right - self.top_left) / 2
        half_height_vec = height * scale_level * unitize(self.bottom_left - self.top_left) / 2
    ...
    def rotate(self, roll=0, pitch=0, yaw=0, degrees=True):
        ...
        if yaw % tau:
            rotvecs.append(unitize(self.top_right - self.top_left) * pitch)
